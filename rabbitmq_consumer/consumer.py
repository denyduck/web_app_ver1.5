...
Session = sessionmaker(bind=engine)         # Session umožňí provádět operace s databází, jako je ukládání, aktualizace nebo mazání dat.
#==========================================================================
...
def callback_outcoming(ch, method, properties, body):
    ...
    file_id,filename, directory, hash, change_type, metadata = extract_message_from_json(message_content)
    print(f'mesage: {file_id}')

    print(f"Extracted message - ID: {file_id}, Filename: {filename}, Directory: {directory}, Hash: {hash}, Type: {change_type}, Metadata: {metadata}")
